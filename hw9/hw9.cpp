...
{ ofstream alfred_hw8("/home/alfred/alfred_hw8.txt");
...
{   
  if((walking_enable_ == true))
  {
    ...
    zmp_measured_LPF_=(2*M_PI*8.0/hz_)/(1+2*M_PI*8.0/hz_)*zmp_measured+1/(1+2*M_PI*8.0/hz_)*zmp_measured_LPF_;
    ...
    if(ready_for_compute_flag_ == true)
    {
      ...
      {      
        ...
        //online_com_trajectory(current_step_num_, com_px, com_py, real_com_x, real_com_y);
        
        //capture_point(current_step_num_, temp_px, temp_py, com_px, com_py, cp_px_eos, cp_py_eos, cp_px, cp_py);
        ...
        else if(current_step_num_ != 0){
        start_time = t_start_;
        }
        alfred_hw9 <<  walking_tick_*0.005 << "," << com_px(walking_tick_-start_time) << "," << com_py(walking_tick_-start_time) << "," << temp_px(walking_tick_-start_time) << "," << temp_py(walking_tick_-start_time) << "," << cp_px(walking_tick_-start_time) << "," << cp_py(walking_tick_-start_time) << "," << cp_px_eos(walking_tick_-start_time) <<"," << cp_py_eos(walking_tick_-start_time) << endl;
        updateNextStepTime();
...
{ 
  ...
  if(current_step_num_==0){
    ...
    {
      if(i >= 0 && i < t_rest_init_ + t_double1_) //0 ~ 0.15\EF\BF????? , 0 ~ 30 tick
      {
        temp_px(i) = 0;
        temp_py(i) = (com_offset_(1) + com_support_init_(1)) + Ky / (t_rest_init_ + t_double1_)* (i+1);
      }
      else if(i >= t_rest_init_ + t_double1_ && i < t_total_ - t_rest_last_ - t_double2_ ) //0.15 ~ 1.05\EF\BF????? , 30 ~ 210 tick
      {
        temp_px(i) = supportfoot_support_init_(0);
        temp_py(i) = supportfoot_support_init_(1);
      }
      else if(i >= t_total_ - t_rest_last_ - t_double2_  && i < t_total_) //1.05 ~ 1.15\EF\BF????? , 210 ~ 230 tick 
      {
...
void WalkingController::online_com_trajectory(unsigned int current_step_num_, Eigen::VectorXd& com_px, Eigen::VectorXd& com_py, Eigen::VectorXd& real_com_x, Eigen::VectorXd& real_com_y)
{
  double Gravity = 9.81;
  double zc = 0.720941;
  double T = 1/hz_;
  ...
  C << 1+zc/Gravity*kp, zc/Gravity*kv;
  Eigen::MatrixXd D(1,1);
  D << -zc/Gravity*kp;
  Eigen::MatrixXd O(2,1);
  O << 0, 0;
  Eigen::Matrix3d A_tilde;
  A_tilde << 1 , C, O, A;
  Eigen::MatrixXd B_tilde(3,1);
  B_tilde << D,B;
  Eigen::MatrixXd I(3,1);
  I <<1,0,0;
  Eigen::Matrix3d K_tilde;
  K_tilde << 109.958674277691,5989.97568738766,1616.43609293652,
  5989.97568738766,329352.591274065,88875.1403616539,
  1616.43609293652,88875.1403616539,23983.9522432179;
  Eigen::Matrix3d Q;
  Q << 1,0,0,0,0,0,0,0,0;
  ...
  double R = 1;
  Eigen::MatrixXd L(1,1);
  L = B_tilde.transpose()*K_tilde*B_tilde;
  L(0,0) = L(0,0) + R;
  Eigen::MatrixXd G_i(1,1);
  G_i = L.inverse()*B_tilde.transpose()*K_tilde*I;
  Eigen::MatrixXd G_x(1,2);
  G_x = L.inverse()*B_tilde.transpose()*K_tilde*F;
  Eigen::Matrix3d A_c;
  A_c = A_tilde-B_tilde*L.inverse()*B_tilde.transpose()*K_tilde*A_tilde;
  Eigen::MatrixXd X(3,1);
  X = -A_c.transpose()*K_tilde*I;
  Eigen::VectorXd G_d;
  G_d.resize(320);
  G_d(0) = - G_i(0,0); 
  ...
  for(int i=1; i<320; i++){
    Eigen::MatrixXd GG_d(1,1);
    GG_d = L.inverse()*B_tilde.transpose()*X;
    G_d(i) = GG_d(0,0);
    X = A_c.transpose()*X;
  }
  ...
  com_px.resize(time_for_step);
  com_py.resize(time_for_step);
  real_com_x.resize(time_for_step);
  real_com_y.resize(time_for_step);
  double dUx;
  double dUy;
  double error_x = 0;
  double error_y = 0;
  ...
  double preview_y = 0;
  //initialize
  if(walking_tick_ == 0 && current_step_num_ == 0){
    com_px(walking_tick_-start_time) = temp_px(0);
    com_py(walking_tick_-start_time) = temp_py(0);
    X_tilde(0,0) = 0;
    X_tilde(1,0) = 0;
    X_tilde(2,0) = 0;
    Y_tilde(0,0) = 0;
  ...
  real_com_x(walking_tick_-start_time) = com_support_current_(0);
  real_com_y(walking_tick_-start_time) = com_support_current_(1);
  //update
  for(int j=0; j<320; j++){
    preview_x = preview_x + G_d(j)*(temp_px(walking_tick_-start_time+j+1)-temp_px(walking_tick_-start_time+j));
    preview_y = preview_y + G_d(j)*(temp_py(walking_tick_-start_time+j+1)-temp_py(walking_tick_-start_time+j));
  }
  error_x = com_support_current_(0) - zc/Gravity*X_tilde(2,0)*hz_ - temp_px(walking_tick_-start_time);
  error_y = com_support_current_(1) - zc/Gravity*Y_tilde(2,0)*hz_ - temp_py(walking_tick_-start_time);
  dUx = -G_i(0,0)*error_x-G_x(0,0) * X_tilde(1,0) - G_x(0,1) * X_tilde(2,0)-preview_x;
  dUy = -G_i(0,0)*error_y-G_x(0,0) * Y_tilde(1,0) - G_x(0,1) * Y_tilde(2,0)-preview_y;
  if(walking_tick_ == start_time && current_step_num_!=0){
  com_px(walking_tick_-start_time) = com_pos(0);
  com_py(walking_tick_-start_time) = com_pos(1);
  }
  if(walking_tick_!= t_last_){
  com_px(walking_tick_-start_time+1) = com_px(walking_tick_-start_time) + dUx;
  com_py(walking_tick_-start_time+1) = com_py(walking_tick_-start_time) + dUy;
  }
  X_tilde = A_tilde*X_tilde + B_tilde*dUx;
  Y_tilde = A_tilde*Y_tilde + B_tilde*dUy;
  ...
  { 
    ...
    com_pos_prev(0) = com_px(walking_tick_-start_time);
    com_pos_prev(1) = com_py(walking_tick_-start_time);
    com_pos = temp_rot*(com_pos_prev - temp_pos);
    
  }
}

void WalkingController::capture_point(unsigned int current_step_num_, Eigen::VectorXd& temp_px, Eigen::VectorXd& temp_py, Eigen::VectorXd& com_px, Eigen::VectorXd& com_py, Eigen::VectorXd& cp_px_eos, Eigen::VectorXd& cp_py_eos, Eigen::VectorXd& cp_px, Eigen::VectorXd& cp_py)
{
  int time_for_step;
  int start_time;
  if(current_step_num_ == 0){
  ...
  temp_px.resize(time_for_step);
  temp_py.resize(time_for_step);
  com_px.resize(time_for_step);
  com_py.resize(time_for_step);
  cp_px_eos.resize(time_for_step);
  cp_py_eos.resize(time_for_step);
  cp_px.resize(time_for_step);
  cp_py.resize(time_for_step);
  //define cp_eos
  if(current_step_num_==0){
    for(int i=0; i<=t_temp_+t_rest_init_+t_double1_;i++){
      cp_px_eos(i) = 0;
      cp_py_eos(i) = 0;
      if(i>=0 && i<=t_temp_-0.5*hz_){
        cp_py_eos(i) = com_support_init_(1);
      }
    }
    for(int i=t_temp_+t_rest_init_+t_double1_+1; i<time_for_step; i++){
      cp_px_eos(i) = foot_step_support_frame_(current_step_num_,0);
      cp_py_eos(i) = foot_step_support_frame_(current_step_num_,1);
    }
  }

  if(current_step_num_ > 0 && current_step_num_!=total_step_num_-1){
    for(int i=0; i<t_rest_init_+t_double1_;i++){
      cp_px_eos(i) = 0;
      cp_py_eos(i) = 0;
    }
    for(int i=t_rest_init_+t_double1_; i<time_for_step; i++){
      cp_px_eos(i) = foot_step_support_frame_(current_step_num_,0);
      cp_py_eos(i) = foot_step_support_frame_(current_step_num_,1);
    }   
  }
  ...
  // define ex,ey,px,py,prev_ex,prev_ey,cx,cy,prev_cx,prev_cy
  if(current_step_num_==0 && walking_tick_==0){
    cx=com_support_init_(0);
    prev_cx=com_support_init_(0);
    ex=com_support_init_(0);
    prev_ex=com_support_init_(0);
    px=com_support_init_(0);
    cy=com_support_init_(1);
    prev_cy=com_support_init_(1);
    ey=com_support_init_(1);
    prev_ey=com_support_init_(1);
    py=com_support_init_(1);
  }
  //step change
  if(walking_tick_== start_time+t_rest_init_+t_double1_ && current_step_num_!=0){
    Td = time_for_step/hz_;
  }
  if(current_step_num_==0){
    if(walking_tick_==0*hz_){
      Td = 1;
    }
    if(walking_tick_==2.5*hz_){
      Td = 0.5+(t_rest_init_+t_double1_)/hz_;
    }
  ...
  //save
  temp_px(walking_tick_-start_time) = px;
  temp_py(walking_tick_-start_time) = py;
  com_px(walking_tick_-start_time) = cx;
  com_py(walking_tick_-start_time) = cy;
  cp_px(walking_tick_-start_time) = ex;
  cp_py(walking_tick_-start_time) = ey;
  //update
  double Gravity = 9.81;
  double w = sqrt(Gravity/zc_);
  double e = 2.718281828459045;
  double d = pow(e, w*Td);
  px = 1/(1-d)*cp_px_eos(walking_tick_-start_time)-d/(1-d)*ex;
  py = 1/(1-d)*cp_py_eos(walking_tick_-start_time)-d/(1-d)*ey;
  ex = px + pow(e, w/hz_)*(prev_ex-px);
  ey = py + pow(e, w/hz_)*(prev_ey-py);
  cx = (w/hz_)/(1+w/hz_)*ex+1/(1+w/hz_)*prev_cx;
  cy = (w/hz_)/(1+w/hz_)*ey+1/(1+w/hz_)*prev_cy;
  if(Td>=0.05+1/hz_){
    Td -= 1/hz_;
  }
  prev_ex = ex;
  ...
  if(walking_tick_ == t_last_ && current_step_num_ != total_step_num_-1)  
  { 
    Eigen::Vector3d before_step;
    Eigen::Vector3d after_step;
    Eigen::Vector3d temp_pos;
    Eigen::Matrix3d temp_rot;
    temp_rot = DyrosMath::rotateWithZ(-foot_step_support_frame_(current_step_num_,5)); 
    for(int i=0; i<3; i++){
    ...
    before_step(0) = cx;
    before_step(1) = cy;
    after_step = temp_rot*(before_step - temp_pos);
    cx = after_step(0);
    cy = after_step(1);
    ...
    before_step(0) = prev_cx;
    before_step(1) = prev_cy;
    after_step = temp_rot*(before_step - temp_pos);
    prev_cx = after_step(0);
    prev_cy = after_step(1);
    
    before_step(0) = prev_ex;
    before_step(1) = prev_ey;
    after_step = temp_rot*(before_step - temp_pos);
    prev_ex = after_step(0);
    prev_ey = after_step(1);
  }
}

void WalkingController::capture_point_tracking(unsigned int current_step_num_, Eigen::VectorXd& temp_px, Eigen::VectorXd& temp_py, Eigen::VectorXd& com_px, Eigen::VectorXd& com_py, Eigen::VectorXd& cp_px_eos, Eigen::VectorXd& cp_py_eos, Eigen::VectorXd& cp_px, Eigen::VectorXd& cp_py)
{
  int time_for_step;
  int start_time;
  if(current_step_num_ == 0){
  ...
  double e = 2.718281828459045;
  double d = pow(e, w*Td);
  px = 1/(1-d)*cp_px_eos(walking_tick_-start_time+1)-d/(1-d)*ex; ///now Ex_d(k+1)
  py = 1/(1-d)*cp_py_eos(walking_tick_-start_time+1)-d/(1-d)*ey;
