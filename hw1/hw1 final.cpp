...
{ 
  ofstream alfred_graph("/home/alfred/alfred_graph.txt");
  ...
  if((walking_enable_ == true))
  {
    ...
    if(ready_for_compute_flag_ == true)
    {
      ...
      {                  
        ...
        double A = 0.3713;
        double B = 0.3728;
        // double hip_offset = 24.08/(2*M_PI);
        // double knee_offset = -14.82/(2*M_PI);
        // double ankle_offset = -9.26/(2*M_PI);
 
        // start with left foot
        Eigen::Vector3d lhip_pose(0, 0.105, -0.1119);
        lhip_pose = pelv_trajectory_float_.translation() + pelv_trajectory_float_.linear()*lhip_pose;
        Eigen::Vector3d lr;
        Eigen::Isometry3d lfoot_inverse = DyrosMath::inverseIsometry3d(lfoot_trajectory_float_);  
        lr = lfoot_inverse.linear()*(lhip_pose - lfoot_trajectory_float_.translation());
 
        double C = sqrt(pow(lr(0), 2) + pow(lr(1), 2) + pow(lr(2), 2));
        ...
        //double q5 = -acos((pow(A,2) + pow(B,2) - pow(C,2))/(2*A*B)) + M_PI;
        double alpha = asin(A*sin(M_PI - q5)/C);
        double q7 = atan2(lr(1), lr(2));
        int sgnz = 1;
        ...
        double q6 = -atan2(lr(0), sgnz*sqrt(pow(lr(1),2) + pow(lr(2),2))) - alpha;
        Eigen::Isometry3d pelv_inverse = DyrosMath::inverseIsometry3d(pelv_trajectory_float_);
        Eigen::Matrix3d R;
        Eigen::Matrix3d Rx = DyrosMath::rotateWithX(-q7);
        Eigen::Matrix3d Ry = DyrosMath::rotateWithY(-q6-q5);
 
        R = pelv_inverse.linear()*lfoot_trajectory_float_.linear()*Rx*Ry;
        double q2 = atan2(-R(0,1), R(1,1));
        double q3 = atan2(R(2,1), -R(0,1) * sin(q2) + R(1,1) * cos(q2));
        double q4 = atan2(-R(2,0), R(2,2));
 
        // right foot
        Eigen::Vector3d rhip_pose(0, -0.105, -0.1119);
        rhip_pose = pelv_trajectory_float_.translation() + pelv_trajectory_float_.linear()*rhip_pose;
        Eigen::Vector3d rr;
        Eigen::Isometry3d rfoot_inverse = DyrosMath::inverseIsometry3d(rfoot_trajectory_float_);  
        rr = rfoot_inverse.linear()*(rhip_pose - rfoot_trajectory_float_.translation());
 
        C = sqrt(pow(rr(0), 2) + pow(rr(1), 2) + pow(rr(2), 2));
        double c11 = (pow(C,2) - pow(A,2) - pow(B,2))/(2*A*B);
 
        double q11; //right knee
        if(c11 >= 1)
        {
          q11 = 0.0;
        }
        ...
        q11 = -1*q11;
        //double q11 = -acos((pow(A,2) + pow(B,2) - pow(C,2))/(2*A*B)) + M_PI;
        alpha = asin(A*sin(M_PI - q11)/C);
        double q13 = atan2(rr(1), rr(2));
        sgnz = 1;
        ...
        double q12 = atan2(rr(0), sgnz*sqrt(pow(rr(1),2) + pow(rr(2),2))) - alpha;
        Rx = DyrosMath::rotateWithX(-q13);
        Ry = DyrosMath::rotateWithY(-q11 - q12);
 
        R = pelv_inverse.linear()*rfoot_trajectory_float_.linear()*Rx*Ry;
        double q8 = atan2(-R(0,1), R(1,1));
        double q9 = atan2(R(2,1), -R(0,1) * sin(q2) + R(1,1) * cos(q2));
        double q10 = atan2(-R(2,0), R(2,2));
 
        q_des << q2, q3, q4, q5, q6, q7, q8, q9, q10, q11, q12, q13;
...
void WalkingController::circling_motion()
{
  ...
  // Todo hw 1
  // double period = 4; //s
  // double refresh_rate = 200; //Hz
 
  // double samples = 4*200;
  // double sampling_time = 4/200;
 
  // equation z�� + x�� = 0.05
 
  double time = walking_tick_ * 0.005;
  double radius = 0.05;
  double lz = -radius*sin(M_PI * time / 2);
  double lx = radius*cos(M_PI * time / 2);
  double rz = -radius*sin(M_PI * time / 2 + M_PI);
  double rx = radius*cos(M_PI * time / 2 + M_PI);
  ...
  lfoot_trajectory_float_.translation()(0) = lx;
  lfoot_trajectory_float_.translation()(1) = 0.12782;
  lfoot_trajectory_float_.translation()(2) = lz - 0.76548; 
 
  //  = Eigen::Isometry3d left(lx, 0.12782, lz - 0.76548);
  rfoot_trajectory_float_.linear().setIdentity();
  rfoot_trajectory_float_.translation()(0) = rx;
  rfoot_trajectory_float_.translation()(1) = -0.12782;
  rfoot_trajectory_float_.translation()(2) = rz - 0.76548;
 
  alfred_graph << lfoot_trajectory_float_.translation()(0)<< ',' << lfoot_trajectory_float_.translation()(1) << ',' << lfoot_trajectory_float_.translation()(2) << endl;
